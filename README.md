...
3. 确保你没有直接双击 html 文件打开，而是通过 Live Server 运行。
...
## 自定义图片功能
下载整个文件之后点击 assets 文件夹，将想要展示的图片导入其中，并按规律进行命名如：1.png/1.jpg 以此类推。
现在照片配置位于 `config.js`，用文本编辑器打开它后更新 `IMAGE_URLS` 数组即可，`PHOTO_COUNT` 会自动跟随长度变化。
